...
def main():
    ...
    parser.add_argument("-c", "--config", required=True, help="AMI config")
    ...
    dated_target_name = "spot-%s-%s" % (
        args.config, time.strftime("%Y-%m-%d-%H-%M", time.gmtime()))
    filters = {
        "tag:moz-state": "ready",
        "instance-state-name": "stopped"
    }
    for tag, value in config["target"]["tags"].iteritems():
    ...
    while True:
        try:
            tmp_v.attach(host_instance.id, config['target']['aws_dev_name'])
            break
        except:
    ...
    with cd(mount_point):
        put("%s/setup_hostname.sh" % AMI_CONFIGS_DIR, "etc/setup_hostname.sh",
